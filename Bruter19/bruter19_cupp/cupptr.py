...
def print_to_file(filename, unique_list_finished):
    ...
    f.close()
    print(
        "[+] Wordlist Oluşturuldu: \033[1;31m"
        + filename
        + "\033[1;m, Kelime Sayısı: \033[1;31m"
        + str(lines)
        + " words.\033[1;m"
    )
...
def interactive():
    ...
    print("\r\n[+] Kişi Hakkında Bilgileri Doğru Girin.")
    print("[+] Eğer Kişi Hakkında Bilmediğiniz Bilgiler Varsa Enter'a Basıp Boş Bırakın ;)\r\n")

    # We need some information first!

    profile = {}

    name = input("bruter19#> Kişinin Adı: ").lower()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] En Azından Bir İsim Girmen Lazım")
        name = input("bruter19#> Kişinin Adı: ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("bruter19#> Soyadı: ").lower()
    profile["nick"] = input("bruter19#> Nickname: ").lower()
    birthdate = input("bruter19#> Doğrum Tarihi (DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] Doğum Tarihi İçin En Az 8 Digit Girmek Zorundasın!")
        birthdate = input("bruter19#> Doğum Tarihi (DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("bruter19#> Karısının/Kocasının) Adı: ").lower()
    profile["wifen"] = input("bruter19#> Karısının/Kocasının) Soyadı: ").lower()
    wifeb = input("bruter19#> Karısının/Kocasının) Doğum Tarihi (DDMMYYYY): ")
    while len(wifeb) != 0 and len(wifeb) != 8:
        print("\r\n[-] Doğum Tarihi İçin 8 Karakter Girmen Lazım!")
        wifeb = input("bruter19#> Karısının/Kocasının Doğum Tarihi (DDMMYYYY): ")
    profile["wifeb"] = str(wifeb)
    print("\r\n")

    profile["kid"] = input("bruter19#> Çocuğunun Adı: ").lower()
    profile["kidn"] = input("bruter19#> Çocuğunun Soyadı: ").lower()
    kidb = input("bruter19#> Çocuğunun Doğum Tarihi (DDMMYYYY): ")
    ...
    profile["pet"] = input("bruter19#> Evcil Hayvanının Adı: ").lower()
    profile["company"] = input("bruter19#> Şirketinin Adı: ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "bruter19#> Saldıracağınız Kişi Hakkında Anahtar Kelimeler Eklemek İster Misiniz? Y/[N]: "
    ).lower()
    words2 = ""
    if words1 == "y":
        words2 = input(
            "bruter19#> Kelimeleri Virüg İle Ayırarak Girin. [örnek: hacker,juice,black], boşluklar silinecektir. "
        ).replace(" ", "")
    profile["words"] = words2.split(",")

    profile["spechars1"] = input(
        "bruter19#> Kelimelerin Sonuna Özel Karakterler Eklensin mi? Y/[N]: "
    ).lower()

    profile["randnum"] = input(
        "bruter19#> Kelimelerin Sonuna Randomlamasına Numaralar Eklensin Mi? Y/[N]:"
    ).lower()
    profile["leetmode"] = input("bruter19#> Leet modu? (örnek: leet = 1337) Y/[N]: ").lower()
